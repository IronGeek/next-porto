 ...
 *
 * Adapted to TypeScript from Sean McCullough's <banksean@gmail.com>
...
class ClassicalNoise {
  readonly grad3: Tuple3D[];
  readonly p: number[];
  readonly perm: number[];

  constructor(r: RNG = Math) { // Classic Perlin noise in 3D, for comparison
    this.grad3 = [
  ...
  dot(g: Tuple3D, x: number, y: number, z: number): number {
...
const classicalNoise = new ClassicalNoise(Math);
